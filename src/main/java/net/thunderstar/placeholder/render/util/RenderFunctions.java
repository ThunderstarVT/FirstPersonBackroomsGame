...
public class RenderFunctions {
    public static class Game {
        ...
        public static void render() {
            ...
            List<Thread> sectorThreads = new ArrayList<Thread>();

            // one thread per sector, gotta optimize it a bit more though
                ...
                Thread thread = new Thread(() -> {
                        ...
                        if (wall.w_texture != null) {
                            ...
                            for (int X = 0; X < 320; X++) {
                                if (wall.intersectBool(
                                        ...
                                        player_rotation, X - 160, Player.constants.camera_fov)) {
                                    ...
                                    for (int Y = Math.max(0, top); Y < Math.min(240, bottom); Y++) {
                                        float distance2 = (float) (distance * Math.sqrt((Y-120)*(Y-120) + (Player.constants.camera_fov)*(Player.constants.camera_fov))/Player.constants.camera_fov);
                                        if (written[X][Y]) {
                                            if (distance2 < dist[X][Y]) {
                                                Color c = RenderHelpFunctions.getColor(wall, sector.s_bottom, sector.s_height,
                                                        ...
                                                        player_rotation, distance, X - 160, Y - 120, Player.constants.camera_fov);
                    ...
                    if (sector.s_floorTexture != null) {
                        for (int Y = 120; Y < 240; Y++) {
                            ...
                            for (int X = 0; X < 320; X++) {
                                float distance2 = (float) (distance * Math.sqrt((X-160)*(X-160) + (Player.constants.camera_fov)*(Player.constants.camera_fov))/Player.constants.camera_fov);
                                ...
                                if (written[X][Y]) {
                                    if (distance2 < dist[X][Y]) {
                                        if (RenderHelpFunctions.checkInPolygon(sector.s_vertices, point)) {
                                            int modX = point.x % 256;
                                            int modY = point.y % 256;
                                            ...
                                            Color c = sector.s_floorTexture[modX][modY];
                                ...
                                if (written[X][Y]) {
                                    if (distance2 < dist[X][Y]) {
                                        if (RenderHelpFunctions.checkInPolygon(sector.s_vertices, point)) {
                                            ...
                                            Color c = sector.s_ceilingTexture[modX][modY];
